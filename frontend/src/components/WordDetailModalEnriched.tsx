...
import { X, BookOpen, History, AlertTriangle, MessageCircle, Link2, MapPin, Volume2 } from 'lucide-react';
import { api } from '../services/api';
import type { FlashcardDetail } from '../types';
...
type TabType = 'overview' | 'examples' | 'etymology' | 'false_friends' | 'proverbs' | 'collocations' | 'dialects';
...
export function WordDetailModal({ wordId, onClose }: WordDetailModalProps) {
  ...
  const visibleTabs = tabs.filter(tab => tab.id === 'overview' || (tab.count && tab.count > 0));
          ...
          <div className="h-48 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 relative overflow-hidden">
            ...
            <div className="absolute bottom-4 left-6 right-6">
              <div className="flex items-center gap-3 mb-2">
                {word.cefr_level && (
                  <span className={`px-3 py-1 rounded-full text-sm font-bold ${CEFR_COLORS[word.cefr_level]}`}>
                    {word.cefr_level}
                  </span>
                )}
                {word.gender && GENDER_LABELS[word.gender] && (
                  <span className={`${GENDER_LABELS[word.gender].color} text-white px-3 py-1 rounded-full text-sm font-bold`}>
            ...
            {visibleTabs.map(tab => (
              ...
              >
                ...
                {tab.count !== undefined && tab.count > 0 && (
                  <span className="bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 text-xs px-2 py-0.5 rounded-full">
                    {tab.count}
                  </span>
                )}
              </button>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {word.examples.slice(0, 2).map((example, index) => (
                      <div key={index} className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-4">
                ...
                <div key={index} className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-5">
                  ...
                  <div className="flex gap-2 mt-3">
                    {example.difficulty_level && (
                      <span className={`text-xs px-2 py-1 rounded-full ${CEFR_COLORS[example.difficulty_level] || 'bg-gray-100 dark:bg-slate-600 text-gray-700 dark:text-gray-300'}`}>
                        {example.difficulty_level}
                      </span>
                    )}
                    {example.context_type && (
                      <span className="text-xs px-2 py-1 rounded-full bg-gray-100 dark:bg-slate-600 text-gray-700 dark:text-gray-300">
                        {example.context_type}
                      </span>
                    )}
                  </div>
          ...
          {activeTab === 'false_friends' && (
            <div className="space-y-4">
              {word.false_friends.map((ff, index) => (
                <div key={index} className="bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-500 rounded-xl p-5">
                  <div className="flex items-start justify-between">
                    ...
                    {ff.confusion_level && (
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${CONFUSION_COLORS[ff.confusion_level] || 'bg-gray-500 text-white'}`}>
              ...
              {word.proverbs.map((proverb, index) => (
                <div key={index} className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-5">
                  ...
                  {proverb.expression_type && (
                    <span className="inline-block mt-2 text-xs px-2 py-1 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400">
                      {proverb.expression_type}
                    </span>
                  )}
                </div>
