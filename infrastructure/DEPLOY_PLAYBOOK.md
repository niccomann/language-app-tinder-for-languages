...
6. **Feedback storage** → in produzione `feedback_service.py` usa il database applicativo persistente (`language-postgres` via `backend.prod.env`). Il fallback JSONL è solo per sviluppo locale quando il database non è disponibile; in prod `ENV=prod` fa fallire esplicitamente il salvataggio se il DB non risponde, evitando path container read-only come `/app`.
